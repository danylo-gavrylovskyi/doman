...
import { useGetCategoryFilterAttributes } from "@/hooks/categories.hooks";
...
export const CategoryListing = ({ categoryId }: { categoryId: number }) => {
	...
	const { data: products } = useGetProductsWithPagination(
		{ page, perPage, categoryId, filterParams: checkedAttributes },
		{ enabled: !!categoryId }
	);

	const { data: filterAttributes } = useGetCategoryFilterAttributes(categoryId);
