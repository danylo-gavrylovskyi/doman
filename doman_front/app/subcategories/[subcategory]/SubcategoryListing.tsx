...
import { useGetSubcategoryFilterAttributes } from "@/hooks/subcategories.hooks";
...
export const SubcategoryListing = ({ subcategoryId }: { subcategoryId: number }) => {
	...
	const { data: products } = useGetProductsWithPagination(
		{ page, perPage, subcategoryId, filterParams: checkedAttributes },
		{ enabled: !!subcategoryId }
	);

	const { data: filterAttributes } = useGetSubcategoryFilterAttributes(subcategoryId);

	if (!products || !filterAttributes) {
		return <SkeletonPage />;
	}
	...
	return (
		<>
			...
			<footer>
				<Pagination elementsCount={products.count} perPage={perPage} currentPage={page} />
			</footer>
		</>
	);
};
